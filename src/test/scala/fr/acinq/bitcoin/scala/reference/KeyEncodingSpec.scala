package fr.acinq.bitcoin.scala.reference

import fr.acinq.bitcoin.scala.Crypto.PrivateKey
import fr.acinq.bitcoin.scala.Base58.Prefix
import fr.acinq.bitcoin.scala.Crypto.PrivateKey
import fr.acinq.bitcoin.scala.{Base58, Base58Check, OP_CHECKSIG, OP_DUP, OP_EQUAL, OP_EQUALVERIFY, OP_HASH160, Script}
import fr.acinq.bitcoin.scala.{Base58, Base58Check, Bech32, OP_CHECKSIG, OP_DUP, OP_EQUAL, OP_EQUALVERIFY, OP_HASH160, OP_PUSHDATA, Script}
...
object KeyEncodingSpec {
  def isValidBase58(input: String): Boolean = Try {
    ...
    prefix match {
      case Prefix.SecretKey | Base58.Prefix.SecretKeyTestnet => Try(PrivateKey.fromBin(bin)).isSuccess
