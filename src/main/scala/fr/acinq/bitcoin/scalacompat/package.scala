...
package object scalacompat {
  ...
  def addressToPublicKeyScript(chainHash: BlockHash, address: String): Either[BitcoinError, Seq[ScriptElt]] = eitherkmp2either(fr.acinq.bitcoin.Bitcoin.addressToPublicKeyScript(chainHash, address))
    .map(l => l.asScala.map(kmp2scala).toSeq)

  def addressFromPublicKeyScript(chainHash: BlockHash, script: Seq[ScriptElt]): Either[BitcoinError, String] = eitherkmp2either(fr.acinq.bitcoin.Bitcoin.addressFromPublicKeyScript(chainHash, script.map(scala2kmp).asJava))
}
